...
BASEDIR = "/home/abe/KidneyM/hubmap2021/MATUI_bbxo/final_G/"
...
with form:
    ...
    with col2:
        ...
        tmp = df[df["path"]==BASEDIR+current_image_file.split("/")[-1]]
        target__ = tmp["target"].values[0]
        dbp = tmp["DBP"].values[0]
        sbp = tmp["収縮期血圧"].values[0]
        egfr = int(tmp["egfr"].values[0])
        ubp = round(tmp["uprot"].values[0],4)
        hep = tmp["血尿"].values[0]
        hep = hep_dict[hep]
        caption_text= f"診断:{target__} ,血圧:{sbp}/{dbp}mmHg,eGFR:{egfr},尿蛋白:{ubp}g/gCr,血尿:{hep}"
        st.image(resized_image, use_container_width=True)
        st.markdown(f"<div style='text-align: center; font-size: 18px; font-weight: bold;'>{caption_text}</div>", unsafe_allow_html=True)
